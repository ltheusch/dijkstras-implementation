...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    public int getWeight(T source, T target) {
        ...
        Vertex sourceVertex = vertices.get(source);
        Vertex targetVertex = vertices.get(target);
        if(sourceVertex == null || targetVertex == null) throw new IllegalArgumentException("Cannot"
            + " retrieve weight of edge between vertices that do not exist");
        for(Edge e : sourceVertex.edgesLeaving)
            if(e.target == targetVertex)
                return e.weight;
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        //official shortest path to be returned, starting with start vertex
        Path shortPath = null;

        if (!containsVertex(start) || !containsVertex(end)) {
            throw new NoSuchElementException("No vertex containing start or end data can be found.");
        } if (start == null || end == null) {
            throw new NoSuchElementException("Cannot calculate with start/end data being null.");
        } if(end.equals(start)){
            //the completed path is only one vertex (both start & end)
            return new Path(vertices.get(start));
        }
        //Hold the discovered paths:
        PriorityQueue<Path> discoveredPath = new PriorityQueue<>();
        //Hashtable to hold visited vertex
        Hashtable<T, Boolean> visited = new Hashtable<>();

        //starting path vertices
        Path starting = new Path(vertices.get(start));

        //add starting vertex to data path
        discoveredPath.add(starting);

        while(!discoveredPath.isEmpty()){
            //current vertex with shortest path
            Path currVert = discoveredPath.remove();
            //add to visited list
            visited.put(currVert.end.data, true);
              //  System.out.println("Visited: "+ currVert.end.data);

             //iterate through edgesLeaving from currVert
            for(Edge edges: currVert.end.edgesLeaving){
                //new path to be added
                Path newPath = new Path(currVert, edges);
                //node has been seen only
                if(visited.containsKey(edges.target.data)){
                    //node has not yet been visited
                    if(!visited.get(edges.target.data)){
                        discoveredPath.add(newPath);
                    }else{
                ...
                if(edges.target.data.equals(end)){
                    //set to shortest if no short path, or if smaller than current
                    //shortest path
                    if(shortPath == null ||newPath.compareTo(shortPath) < 0){
                        shortPath = newPath;
                    }
                    //otherwise path remains the same
                }
            }
        }
        //shortPath has not been updated at all - error?
        if(shortPath == null){
            throw new NoSuchElementException("Error: no path found.");
        }
        //shortest path calculated returned!!
        return shortPath;
